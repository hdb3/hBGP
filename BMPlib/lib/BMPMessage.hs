...
data BMPMsg
  = BMPRouteMonitoring RouteMonitoring
...
instance Show PerPeerHeader where
  show PerPeerHeader {..} =
    "PerPeerHeader { pphAddress = " ++ show pphAddress
      ++ ", pphAS = "
      ++ show pphAS
      ++ ", pphBGPID = "
      ++ show pphBGPID
      ++ ", pphTimeStampSecs = "
      ++ show pphTimeStampSecs
      ++ ", flags = "
      ++ if vFlag
        then "V"
        else
...
showInitiationTLVs = unlines . map showInitiationTLV
  where
...
data BMPPeerDownMsg = BMPPeerDownMsg
  { pph :: PerPeerHeader,
    reason :: Word8,
...
data BMPPeerUPMsg = BMPPeerUPMsg
  { pph :: PerPeerHeader,
    localAddress :: IP.IP,
...
data BMPPeerStatsMsg = BMPPeerStatsMsg
  { pph :: PerPeerHeader,
    stats :: [TLV]
  }

instance Show BMPPeerStatsMsg where
  show BMPPeerStatsMsg {..} =
    "BMPPeerStatsMsg { pph = " ++ show pph
      ++ ", stats = "
      ++ show stats
      ++ " }"
