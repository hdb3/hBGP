...
checkPrefixes st =
  if cPrefixes == ribPrefixes
    then Nothing
    else
      Just $
        "checkPrefixes FAIL: counts - "
          ++ show (length cPrefixes)
          ++ " / "
          ++ show (length ribPrefixes)
          ++ "\n "
...
checkGroups st =
  if cGroups == ribGroups
    then Nothing
    else
      Just $
        "checkGroups FAIL: counts - "
          ++ show (length cGroups)
          ++ " / "
          ++ show (length ribGroups)
          ++ "\n "
