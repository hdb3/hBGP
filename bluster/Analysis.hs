...
basicAnalysis st@State {..} =
  "clusters: " ++ show clusterCount
    ...
    ++ "\n"
    ++ "singleton prefixes: "
    ++ show singletonPrefixes
    ++ "  simple clusters: "
    ++ show simpleClusters
    ++ "  (prefixes: "
    ++ show simpleClusterPrefixCount
    ++ "  avg size: "
    ++ showF (fromIntegral simpleClusterPrefixCount / fromIntegral simpleClusters)
    ++ ") % simple/single = "
    ++ showF (100.0 * fromIntegral (simpleClusterPrefixCount + singletonPrefixes) / fromIntegral prefixCount)
    ++ "%\n"
    ++ "acceleration factor: "
    ++ showF accelerationFactor
