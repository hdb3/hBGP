...
zParser role n' = do
  ...
  if
      | cmd == _ZEBRA_HELLO && n == 1 -> do
        protocol <- anyWord8
        return $ ZMHello protocol
      | cmd == _ZEBRA_INTERFACE_ADD ->
        if n == 0
          then return ZMQInterfaceAdd
          else do
            interface <- zInterfaceParser
...
zNextHopParser = do
  ...
  if
      | nextHopType == _ZEBRA_NEXTHOP_BLACKHOLE -> return ZNHBlackhole
