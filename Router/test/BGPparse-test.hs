...
import BGPparse hiding (decodeBGPByteString, encodeBGPByteString)
import Capabilities
import Common
import Data.Binary
...
identityCheck name bgpMsg = do
  putStrLn $ "identityCheck on " ++ name
  let encMsg = myEncode bgpMsg
  print $ simpleHex $ L.toStrict encMsg
  let decMsg = myDecodeOrFail encMsg
  case decMsg of
    (Left (_, _, s)) -> do
      print $ "failed to decode message:" ++ s
      print $ simpleHex $ L.toStrict encMsg
    (Right (_, _, decMsg')) ->
      if decMsg' == bgpMsg
        then do
          print "success"
        else -- print $ simpleHex $ L.toStrict encMsg
        do
          print "recoded message is not identical"
          print bgpMsg
          print $ simpleHex $ L.toStrict encMsg
          print decMsg'
  putStrLn ""
