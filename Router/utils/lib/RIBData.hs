...
-- IntMap is only marginally faster than base Map
...
makeRoute fromEBGP attributes =
  let attributesHash = Data.Hashable.hash attributes
   in Route
...
instance {-# OVERLAPPING #-} Ord (Peer, Route) where
  compare (pd1, rd1) (pd2, rd2) =
    compare
      (localPref rd1, pathLength rd1, origin rd1, med rd1, not $ fromEBGP rd1, peerBGPid pd1, peerIPv4 pd1)
