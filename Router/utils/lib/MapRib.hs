...
data MapRib = MapRib
  { fSel :: (Peer, Route) -> (Peer, Route) -> Ordering,
...
instance Rib MapRib where
  ...
  removePeer peer rib = (actions, newRib)
    where
      ...
      (actions, newRib) = Data.List.foldl' f ([], rib) prefixesForPeer
        where
          f :: RibM MapRib -> Int -> RibM MapRib
          -- f :: ([(Prefix, (Peer,Route))],MapRib) -> Int -> ([(Prefix, (Peer,Route))],MapRib)
          f (ax, r) i =
            let pfx = fromInt i -- note this single definition of pfx is needed to allow the following two lines to be unambiguous (and thus to compile)
            -- the function signature for 'f', above, though is just for documentation ;-)
                (a, r') = withdraw pfx peer r
             in (a : ax, r')

  adjust prefix' peer route rib = (action, rib')
    where
      prefix = toInt prefix'
