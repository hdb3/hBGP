...
import BGPlib.BGPlib(wireFormat,Prefix,BGPMessage(..))
...
wireParse bs = do
    ...
    either (\s -> die $ "BGP wire format parse failed: " ++ s)
...
bgpParse bs = do
    ...
    either (\s -> die $ "BGP message parse failed: " ++ s)
           (\msgs -> do putStrLn $ "read " ++ show (length msgs) ++ " messages from " ++ show (B.length bs) ++ " bytes"
