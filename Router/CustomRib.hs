...
module Router.CustomRib (ribPull, msgTimeout, addRouteRib, delRouteRib, updateFromAdjRibEntrys, routesFromAdjRibEntrys, delPeerByAddress, addPeer, ribPush, RibHandle) where

import BGPRib.BGPRib hiding (addPeer, ribPush)
...
getDeltaFirstPull rh =
  modifyMVar
    (mvCRib rh)
    ( \crib ->
        if zeroDay == firstPull crib
          then getCurrentTime >>= \t -> return (crib {firstPull = t}, fromRational 0.0)
          else getCurrentTime >>= \t -> return (crib, diffUTCTime t (firstPull crib))
    )
...
addPeer _ peer = do
  ...
  let testMode = getVal dict Passive "testMode"
...
ribPush RibHandle {..} update = do
  ...
  if active cRib
    then putMVar mvCRib (cRib {msgCount = mc, lastUpdate = now})
    else putMVar mvCRib (cRib {msgCount = 1, active = True, firstUpdate = now, lastUpdate = now})
  ...
  if null updates
    then do
      when
        pullActive
        ( do
            info "ribPull: update stream ended"
