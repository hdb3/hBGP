...
import Data.Attoparsec.Lazy (Parser, anyWord8, count, word8)
import qualified Data.Attoparsec.Lazy as DAB
import Data.Bits
      ...
      <$> if null args || "-" == head args
        then BS.getContents
          ...
          if "gz" `isSuffixOf` head args
            then decompress <$> BS.readFile (head args)
