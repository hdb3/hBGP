...
import BGPlib.GetBGPMsg(hSndRawMessage, hSndRawMessages, hGetRawMsg)
...
bgpSnd (BGPHandle h) msg | 4079 > lengthEncodedMsg = catchIOError ( hSndRawMessage h encodedMsg )
...
bgpSndAll (BGPHandle h) msgs = catchIOError ( hSndRawMessages h $ map encode msgs )
                                            (\e -> throw $ BGPIOException (show (e :: IOError)))

bgpRcv :: BGPHandle -> Int -> IO BGPMessage
bgpRcv (BGPHandle h) t | t > 0     = fmap decodeBGPByteString (hGetRawMsg h t)
